...
uv run pytest              # full test matrix (~55 tests)
```
...
- No automated formatter / linter is wired up yet (intentional, see
  `docs/superpowers/specs/2026-05-24-yapi-v2.1-design.md` §10). Match the
...
4. Examples in `examples/` still import without errors:
   ```bash
   uv run python -c "import examples.wish_api, examples.mixed_router, examples.with_depends, examples.custom_runner"
   ```
...
`docs/superpowers/specs/2026-05-24-yapi-v2.1-design.md` §10 for the full
list of out-of-scope items before proposing large features.
