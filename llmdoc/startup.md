...
- yapi 是什么：prompt-first 的声明式 HTTP 框架，公开符号 8 个：`PromptRouter / PromptContext / AgentRunner / RunnerContext / YapiError / YapiDeclarationError / RuntimeExecutionError / YapiUsageWarning`（`yapi/__init__.py` `__all__`）。
- 当前实现对齐 v2.2 spec（`docs/superpowers/specs/2026-05-24-yapi-v2.2-design.md`，在 v2.1 基础上增量扩展，引入 `PromptContext` 与 `<context>` XML 边界）；v0 plan 已过时，不要按它写代码。
...
- 函数签名 4 分类（`ParamRole`）：**REQUEST_MODEL**（裸 BaseModel 或 `Annotated[BaseModel, Body()]`）、**DEPENDENCY**（`Depends()` 默认值或 `Annotated[T, Depends()]`）、**INJECTED_FIELD**（`Query/Header/Cookie/Path/Form/File` 作 default 或 Annotated metadata）、**PROMPT_CONTEXT**（裸 `PromptContext` 注解，v2.2 新增，自动注入 + 从 FastAPI 看到的 `__signature__` 中过滤）；至多一个 REQUEST_MODEL、至多一个 PROMPT_CONTEXT；返回注解必须是 `BaseModel` 子类。
- 用户函数 `return` 仍是 `None | str`：`str` 段会与 `PromptContext` 收集的片段一起裹进 `<context>...</context>` 作为 system prompt 最后一段；无任何段时整段省略。
